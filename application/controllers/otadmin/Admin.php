...
class Admin extends Admin_Controller {
	//Config Module
	var $title = 'Quản trị';
	var	$path_url = 'otadmin/admin/';
	var	$table = 'tbl_admin';
	var	$template = 'backend/admin/';
	var	$control = 'admin';
	...
	public function index()
	{
		...
		$data['datas'] = $this->query_sql->select_array($this->table,'*',array('role_id' => 1),'id desc');
	...
	public function add()
	{
		...
		$data['role'] = $this->query_sql->select_array('tbl_role','id,name',array('publish' => 1));
		$this->load->view('backend/default/index', isset($data)?$data:NULL);
	}
	...
	public function edit2()
	{
		if($this->CI_auth->check_logged() === false){redirect(base_url().'otadmin/login.html');}
		$data['title'] = 'Cập nhật dữ liệu';
		$data['template'] = $this->template.'edit2';
		$data['path_url'] = $this->path_url;
		$data['data_index'] = $this->get_index();
		//edit
		if($this->input->post()){
			...
			$result = $this->query_sql->edit($this->table, $data_update, array('id' => $this->CI_auth->logged_id()));
			if($result>0){
				$this->session->set_flashdata('message_flashdata', array(
					'type'		=> 'sucess',
					'message'	=> 'Cập nhật dữ liệu thành công!!',
