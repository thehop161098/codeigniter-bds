...
class Showroom extends Admin_Controller {
	//Config Module
	var $title = 'Showroom';
	var $path_dir = 'upload/showroom/';
	var	$path_dir_thumb = 'upload/showroom/thumb/';
	var	$path_url = 'otadmin/showroom/';
	var	$table = 'tbl_showroom';
	var	$template = 'backend/showroom/';
	var	$control = 'showroom';
	...
	public function index()
	{
		...
		if($data['datas'] !=NULL)
		{
			foreach ($data['datas'] as $key_datas => $val_datas) {
				$mapShowroom = $this->query_sql->select_array('tbl_map_showroom','name',array('showroomID'=>$val_datas['id']),'sort desc');
				$data['datas'][$key_datas]['mapShowroom'] = $mapShowroom;
			}
		}
	...
	public function delete()
	{
		if($this->CI_auth->check_logged() === false){redirect(base_url().'otadmin/login.html');}
		$id = $_POST['id'];
		$data = $this->query_sql->select_row('tbl_map_showroom','showroomID',array('showroomID'=>$id),'id desc');
		if($id != $data['showroomID'])
		{
	...
	public function deleteall()
	{
        ...
        foreach ($list_id as $key => $val) {
        	$data = $this->query_sql->select_row('tbl_map_showroom','showroomID',array('showroomID'=>$val),'id desc');
        	if($val != $data['showroomID'])
        	{
